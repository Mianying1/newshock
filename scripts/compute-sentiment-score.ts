...
export async function runComputeSentimentScore(): Promise<ComputeSentimentScoreStats> {
  ...
  console.log(`computing sentiment for ${activeThemes.length} active themes · window=${WINDOW_DAYS}d · ${events.length} events total`)
  ...
  for (const theme of activeThemes) {
    ...
    for (const e of evs) {
      ...
      if (direction === 'supports') {
        ...
        const contribution = w * timeDecay // no recency bonus on bear per spec
    ...
    let dominant: string
    const bull = bullScore, bear = bearScore
    ...
    // Recent signal shift · last 7d top events by weight
    ...
    if (Math.abs(sentiment) > 3 || dominant === 'bearish' || shiftDirection !== 'none' && shiftDirection !== 'balanced') {
      console.log(`  ${dominant.padEnd(8)} score=${clamped.toFixed(2).padStart(6)} bull=${bull.toFixed(2).padStart(5)} bear=${bear.toFixed(2).padStart(5)} · ${theme.name.slice(0, 55)}`)
    }
  }

  console.log(`\ndone · ${updated} themes updated`)
  console.log('\ndistribution:')
