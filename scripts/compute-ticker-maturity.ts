...
// Piecewise-linear normalize: values -> [0, 10] via min=0, median=5, max=10
...
async function main() {
  ...
  const tickers = Array.from(perTicker.values()).map((a) => {
    ...
    // NULL avg_strength (no active theme) → treated as 0 per Q1 decision
  ...
  console.log(`scoring ${scored.length} tickers...`)

  // Write back per ticker: one UPDATE covers all that ticker's recommendations
  let updated = 0
